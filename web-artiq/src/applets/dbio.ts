...
const parse = (str: KeyString): Key | undefined => {
  try {
...
export const keys = (viewtype?: string): Key[] =>
  Object.keys(window.localStorage)
    .map(parse)
    .filter((k): k is Key => k !== undefined)
    .filter((k) => viewtype === undefined || viewtype === k.viewType);

export const write = <T>(v: T): void =>
  window.localStorage.setItem(JSON.stringify(curr()), JSON.stringify(v));

export const read = <T>(): T | undefined => {
  const str = window.localStorage.getItem(JSON.stringify(curr()));
  return str === null ? undefined : JSON.parse(str);
};

export const remove = (k: Key) =>
  window.localStorage.removeItem(JSON.stringify(k));
