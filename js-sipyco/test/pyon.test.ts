...
const hinted = readFileSync(join(__dirname, "pyon_v2.test.json"), "utf8");
const tagged = pyon.decode(hinted);

const normalize = (s: string): string => JSON.stringify(JSON.parse(s));
...
const testCopy = (orig: pyon.TypeTaggedObject) => {
  const copy = pyon.copy(orig);
...
const tupleArrayKey = [1, 2] as any;
...
describe("dict", () => {
  it("should match the structure of a JS pyon dict", () => {
    ...
    expect(pyon.preview(tagged.get("od"))).toBe(
      `["dict",[[2,"a"],[1,"b"],[0,"c"]]]`,
    );
...
describe("tuple", () => {
  it("should match the structure of a JS pyon tuple", () => {
    ...
    expect(pyon.preview(tagged.get(tupleArrayKey))).toBe(
      `[["tuple",[3,4.2]],["tuple",[2]]]`,
    );
  });
});
...
const linspaceKey = new Fraction(3, 4) as any;
...
const complexKey = new Float64Array([0, 1]) as any;
