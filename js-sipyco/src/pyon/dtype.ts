...
export const from = (dtype: Name): TypedArrayConstructor => {
  const entry = dict.find((e: Entry) => e.dtype === dtype);
  if (entry) {
    return entry.ctor;
  }
...
export const toString = (ctor: TypedArrayConstructor): Name | undefined =>
  dict.find((e: Entry) => e.ctor.name === ctor.name)?.dtype;
