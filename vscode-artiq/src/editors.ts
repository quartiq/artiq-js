...
export type Editor<P extends argument.Procdesc> = (
  info: Info<P>,
) => HTMLElement;

export const buttons: Editor<argument.Enum> = (info) => {
  ...
  el.addEventListener(
    "keydown",
    (ev) => ev.key === "Escape" && info.cancel(info.arg[3]),
  );

  return el;
};

export const select: Editor<argument.Enum> = (info) => {
...
export const input: Editor<argument.Procdesc> = (info) => {
