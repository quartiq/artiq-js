...
const join = (prefix: string, unit: string) =>
  prefix === basic ? unit : prefix + unit;
const exp = (prefix: string) =>
  step * (all.indexOf(prefix) - all.indexOf(basic));
...
const exps = Object.entries({
  ...
  .map(([unit, supported]) =>
    [...supported].map((prefix) => [join(prefix, unit), exp(prefix)]),
  )
  .flat();

// returns the corresponding decimal factor for any supported prefixed unit symbol
export const scale = (prefixed: string) =>
  Math.pow(10, Object.fromEntries(exps)[prefixed]);
